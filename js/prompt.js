let intento = 0;
let adivinaBtn = document.getElementById("adivinaBtn");
...
function aleatorio(){
    ...
    let aux = 0;
    do{
        aux = prompt("Ingrese un número de 1 al 100");
        if (numRandom == aux ){
            alert("Muy bien!")
        }
        if (numRandom != aux){
        ...
        if (numRandom < aux){
            alert ("Prueba con un número más bajo")
        }
    }while(aux != numRandom)
    console.log(`Has tardado ${intento} intentos en acertar`);
}
